...
import src.snuffy_module.multi_head_attention as multi_head_attention
import src.snuffy_module.pff as pff
import src.snuffy_module.i_classifier as iclassifier
import src.snuffy_module.b_classifier as bclassifier
import src.snuffy_module.encoder_layer as encoder_layer
import src.snuffy_module.encoder as encoder
import src.snuffy_module.mil_net as mil_net
from utils import check_layers

DATASET_PATH = "/Users/binhnam/Desktop/roadtoFrance/mil-cls-ver1/weight"
...
def test(args, bags_list, milnet):
    ...
    for i in range(0, num_bags):
        ...
        slide_input_path_mask = os.path.join(
            MASKS_PATH, f"{slide_name}_mask.tif"
        )
        ...
        with torch.no_grad():
            ...
            if bag_prediction >= args.thres_tumor:
                print(
                    f"{bags_list[i]} is detected as malignant {1} ({bag_prediction}) | label: {label}"
                )
                color = [1, 0, 0]
            else:
                print(
                    f"{bags_list[i]} is detected as benign {0} ({bag_prediction}) | label: {label}"
                )
                color = [0, 1, 0]
            ...
            # input_mask = input_mask.getUCharPatch(
            #     startX=0,
            #     startY=0,
            ...
            input_slide = input_slide.read_region(
                (0, 0), level, input_image_size
            )
...
def blend_and_visualize(
    ...
    color_map = np.zeros(
        (np.amax(pos_arr, 0)[1] + 1, np.amax(pos_arr, 0)[0] + 1)
    )
    ...
    # mask = mask.filter(ImageFilter.FIND_EDGES).filter(
    #     ImageFilter.MaxFilter(size=17)
    # )
    # mask = np.array(mask)
    # mask = transform.resize(
    #     mask, (input_img_size[1], input_img_size[0]), order=0
    # )
...
def _load_embedder_weights(args, milnet: mil_net.MILNet):
    embedder_state_dict_weights = torch.load(args.embedder_weights, map_location=torch.device('cpu'))
    ...
    check_layers(
        milnet.i_classifier.state_dict(),
        embedder_state_dict_weights,
        header="Embedder",
    )
    milnet.i_classifier.load_state_dict(
        embedder_state_dict_weights, strict=False
    )


def _load_aggregator_weights(args, milnet: mil_net.MILNet):
    aggregator_state_dict_weights = torch.load(args.aggregator_weights, map_location=torch.device('cpu'))
    aggregator_state_dict_weights[
        "i_classifier.fc.weight"
    ] = aggregator_state_dict_weights.pop("i_classifier.fc.0.weight")
    aggregator_state_dict_weights[
        "i_classifier.fc.bias"
    ] = aggregator_state_dict_weights.pop("i_classifier.fc.0.bias")
    check_layers(
        milnet.state_dict(), aggregator_state_dict_weights, header="Aggregator"
    )
...
def get_snuffy_milnet(args):
    ...
    i_classifier = iclassifier.IClassifier(
        resnet, args.feats_size, output_class=args.num_classes
    ) # .cuda()
    ...
    milnet = mil_net.MILNet(i_classifier, b_classifier) # .cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Batch size of feeding patches",
    )
    ...
    parser.add_argument(
        "--embedder_weights",
        type=str,
        default="/Users/binhnam/Desktop/roadtoFrance/mil-cls-ver1/weight/embedding.pth",
    )
    parser.add_argument(
        "--aggregator_weights",
        type=str,
        default="/Users/binhnam/Desktop/roadtoFrance/mil-cls-ver1/weight/snuffy_simclr_dsmil.pth",
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="using the modified type of softmax or not",
    )
    parser.add_argument(
        "--mlp_multiplier",
        default=4,
    ...
    bags_list = list(
        map(
            lambda x: os.path.join(
                "/Users/binhnam/Desktop/roadtoFrance/mil-cls-ver1/weight",
                "1_tumor",
                x,
            ),
            slides,

...
from src.utils_snuffy.froc import mp_computeFROC_list_no_cache
from torch.nn.modules.loss import _Loss
import torch.optim as optim
from torch.autograd import Variable
from src.utils_snuffy.utils import (
...
from src.utils_snuffy.utils import compute_pos_weight, dropout_patches
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="Train MIL Models on patch features learned by the SSL method"
    )
    parser.add_argument(
        "--num_classes",
        default=1,
    ...
    parser.add_argument(
        "--lr", default=2e-4, type=float, help="Initial learning rate [0.0002]"
    )
    parser.add_argument(
        "--num_epochs",
        default=200,
    ...
    parser.add_argument(
        "--dataset", default="camelyon16", type=str, help="Dataset folder name"
    )
    parser.add_argument(
        "--embedding",
        default="SimCLR",
        type=str,
        help="Embeddings to be used for feature computation",
    )
    parser.add_argument(
        "--split",
        default=0.2,
        type=float,
        help="Training/Validation split [0.2]",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--arch", default="snuffy", type=str, help="architecture"
    )
    parser.add_argument(
        "--bins", default=10, type=int, help="number of bins for calibration"
    )
    ...
    parser.add_argument(
        ...
        default=10,
        type=int,
        help="Number of cross validation fold [10]",
    )
    parser.add_argument(
        "--cv_current_fold",
        default=0,
    ...
    parser.add_argument(
        ...
        default=4,
        type=int,
        help="inverted mlp anti-bottbleneck",
    )
    parser.add_argument(
        "--encoder_dropout", default=0.0, type=float, help="dropout in encoder"
    )
    parser.add_argument(
        "--activation",
        default="relu",
        type=str,
        help="activation function used in semi transforer",
    )
    parser.add_argument(
        "--clip_grad",
        default=None,
        type=float,
        help="gradient clipping or not and the way to do it",
    )
    parser.add_argument(
        "--depth",
        default=1,
    ...
    parser.add_argument(
        "--roc_run_name",
        type=str,
        help="Name of the run for which we're saving predictions and labels.",
    )
    parser.add_argument(
        "--roc_run_epoch",
        type=int,
        help="Epoch number of the run for which we're saving predictions and labels.",
    )
    parser.add_argument(
        "--roc_data_split",
        default="test",
...
class Trainer:
    ...
    def _get_criterion(self) -> Optional[_Loss]:
        ...
        self.__is_criterion_set = not (self.args.dataset in MIL_DATASETS)
    ...
    def _get_optimizer(self) -> optim.Optimizer:
        try:
            ...
            raise Exception(
                f"Optimizer not found. Given: {self.args.optimizer}, Have: {OPTIMIZERS.keys()}"
            )
    ...
    def _load_init_weights(self):
        try:
            weight_init_i_part = WEIGHT_INITS[
                self.args.weight_init__weight_init_i__weight_init_b[1]
            ]
            weight_init_b_part = WEIGHT_INITS[
                self.args.weight_init__weight_init_i__weight_init_b[2]
            ]
            ...
            if (
                self.args.weight_init__weight_init_i__weight_init_b[0]
                is not None
            ):
                raise Exception(
    ...
    def train(self, data, cur_epoch):
        ...
        if not self.__is_criterion_set:
            pos_weight = torch.tensor(
                compute_pos_weight(labels), device=device
            )
        ...
        for i in range(num_bags):
            ...
            if self.args.l2normed_embeddings == 1:
                bag_feats = bag_feats / np.linalg.norm(
                    bag_feats, axis=1, keepdims=True
                )
            bag_feats = dropout_patches(bag_feats, self.args.dropout_patch)
            # ------------------------
            bag_label = Variable(
                Tensor(np.array([bag_label])).to(device)
            )  # .unsqueeze(dim=0)
            bag_feats = Variable(
                Tensor(np.array([bag_feats])).to(device)
            )  # .unsqueeze(dim=0)
            # ------------------------
            bag_prediction, loss, attentions = self._run_model(
                bag_feats, bag_label
            )
            loss.backward()
            # ----------------------------------------
            ...
            self._after_run_model_in_training_mode(
                step=step, num_bags=num_bags, batch_idx=i
            )
            # ----------------------------------------
            ...
            sys.stdout.write(
                "\r Training bag [%d/%d] bag loss: %.4f"
                % (i, num_bags, loss.item())
            )
            with torch.no_grad():
    ...
    def valid(
        self,
        data,
        predefined_thresholds_optimal=None,
        predefined_feats_thresholds_optimal=None,
        plot_prefix=None,
        metric=None,
        mode="valid",
    ):
            ...
            for i in range(num_bags):
                ...
                if is_test and self.args.dataset == "camelyon16":
                    slide_detections = [
                        ...
                        for position, prob in zip(
                            all_positions_int[i],
                            attentions.cpu().numpy().squeeze(),
                        )
                    ]
                    detections.append(slide_detections)

                total_loss = total_loss + loss.item()
                step_validation_metrics = {"step_valid_bag_loss": loss.item()}
                wandb.log(step_validation_metrics)
                sys.stdout.write(
                    "\r Testing bag [%d/%d] bag loss: %.4f"
                    % (i, num_bags, loss.item())
                )
                predictions.extend([bag_prediction])
                if self._should_calc_feats_metrics(data):
        ...
        if self.args.for_roc_curve:
            ...
            labels_predictions_f_path = os.path.join(
                roc_base_dir, f"{self.args.roc_run_epoch}.npz"
            )
            np.savez(
                labels_predictions_f_path,
                labels=labels,
                predictions=predictions,
            )
        ...
        if self._should_calc_feats_metrics(data):
            ...
            ) = self._calc_feats_metrics(
                feat_labels,
                feat_predictions,
                predefined_feats_thresholds_optimal,
            )
        ...
        if (
            self._should_calc_feats_metrics(data)
            and is_test
            and self.args.dataset == "camelyon16"
        ):
            with mp.Pool(self.args.num_processes) as pool:
                res_ = pool.map(
                    mp_thresholding,
                    [
                        (data, feats_thresholds_optimal[0], key)
                        for data, key in zip(detections, all_image_names)
                    ],
                )
    ...
    def _calibration_cal(self, data, metric):
        ...
        # --------------------------
        ece_error = ece_criterion.loss(
            softmaxes, labels_np, 0.5, n_bins=self.args.bins, logits=False
        )
    ...
    def _after_run_model_in_training_mode(self, step, num_bags, batch_idx):
        if self.args.clip_grad is not None:
            torch.nn.utils.clip_grad_norm_(
                self.milnet.parameters(), max_norm=self.args.clip_grad
            )
    ...
    def _calc_metrics(
        self, labels, predictions, predefined_thresholds_optimal=None
    ):
        ...
        auc_scores, _, thresholds_optimal = multi_label_roc(
            labels, predictions, self.args.num_classes
        )
        ...
        if self.args.num_classes == 1:
            ...
            for i in range(self.args.num_classes):
                ...
                class_prediction_bag[
                    predictions[:, i] >= thresholds_optimal[i]
                ] = 1
                class_prediction_bag[
                    predictions[:, i] < thresholds_optimal[i]
                ] = 0
...
class Runner:
    def __init__(self, args, trainer: Trainer):
        ...
        if self.args.dataset in HISTOPATHOLOGY_DATASETS:
            ...
            self.train_data, self.valid_data, self.test_data = load_mil_data(
                args
            )
    ...
    def _set_dirs(self):
        self.save_path = os.path.join(
            SAVE_PATH, self.args.dataset, wandb.run.name
        )
    ...
    def _get_data(self):
        ...
        path_prefix = os.path.join(
            ".", EMBEDDINGS_PATH, self.args.dataset, self.args.embedding
        )
        ...
        if self.args.dataset == "camelyon16":
            train_df, valid_df, test_df = self._get_dataframe_splits_by_folder(
                bags_df, path_prefix
            )
        ...
        print(
            f"Num Bags (Train: {len(train_df)}) (Valid: {len(valid_df)}) (Test: {len(test_df)})"
        )
    ...
    def _get_official_data(self):
        bags_csv = os.path.join(
            EMBEDDINGS_PATH,
            self.args.dataset,
            "official",
            f"{self.args.dataset.capitalize()}.csv",
        )
        bags_df = pd.read_csv(bags_csv)
        train_df, valid_df, test_df = self._get_dataframe_splits_by_args(
            bags_df
        )
    ...
    def _get_dataframe_splits_by_folder(self, bags_df, path_prefix):
        ...
        dataframe_splits = (
            bags_df[bags_df["0"].str.startswith(f"{path_prefix}/{split_name}")]
            for split_name in split_names
        )
        return dataframe_splits

    def _get_dataframe_splits_by_args(self, bags_df):
        train_df = bags_df.iloc[
            0 : int(len(bags_df) * (1 - self.args.split)), :
        ]
    ...
    def _log_initial_metrics(self):
        ...
        initial_metrics_file_path = os.path.join(
            self.save_path, f"initial_results.txt"
        )
    ...
    def _load_epoch_model(self, epoch: int):
        ...
        single_weight_parameter_save_path = os.path.join(
            self.save_path, f"single_weight_parameter_{epoch}"
        )

        self.trainer.milnet.load_state_dict(
            torch.load(model_save_path), strict=True
        )

        with open(log_save_path, "r") as f:
            epoch_valid_metrics = json.load(f)
        thresholds_optimal = np.asarray(
            a=eval(epoch_valid_metrics["thresholds_optimal"]), dtype=np.float32
        )
        report = f"Using thresholds_optimal: {thresholds_optimal}"

        feats_thresholds_optimal = epoch_valid_metrics[
            "feats_thresholds_optimal"
        ]
        if feats_thresholds_optimal is not None:
            feats_thresholds_optimal = np.asarray(
                a=eval(feats_thresholds_optimal), dtype=np.float32
            )
        ...
        if hasattr(self.trainer, "single_weight_parameter"):
            ...
            self.trainer.single_weight_parameter = torch.load(
                single_weight_parameter_save_path
            )
        print(report)
    ...
    def _save_epoch_model(
            ...
            json.dump(
                {
                    ...
                    "feats_thresholds_optimal": str(feats_thresholds_optimal)
                    if feats_thresholds_optimal is not None
                    else None,
                },
                f,
        ...
        if hasattr(self.trainer, "single_weight_parameter"):
            ...
            torch.save(
                self.trainer.single_weight_parameter,
                single_weight_parameter_save_path,
            )
        ...
        if should_log_report:
            print(
                f"\t[{report_prefix}] {model_report} {thresholds_report} {single_weight_parameter_report}"
            )
    ...
    def run_train(self):
        ...
        for epoch in range(1, self.args.num_epochs + 1):
            ...
            thresholds_optimal = epoch_valid_metrics[
                "epoch_valid_thresholds_optimal"
            ]
            feats_thresholds_optimal = epoch_valid_metrics[
                "epoch_valid_feats_thresholds_optimal"
            ]
            epoch_train_time = int(
                start_valid_epoch_time - start_train_epoch_time
            )
            epoch_valid_time = int(
                end_valid_epoch_time - start_valid_epoch_time
            )
            ...
            print(
                ...
                + "|".join(
                    "class-{0}>>{1:.4f}".format(*k)
                    for k in enumerate(valid_aucs)
                )
            )
        ...
        with open(
            os.path.join(self.save_path, "train_metrics.json"), "w"
        ) as f:
    ...
    def run_test(self, best_auc_epochs):
        ...
        for epoch, plot_prefix in special_epochs:
            ...
            (
                thresholds_optimal,
                feats_thresholds_optimal,
            ) = self._load_epoch_model(epoch)
            epoch_test_metrics = self.trainer.test(
                self.test_data,
                thresholds_optimal,
                feats_thresholds_optimal,
                plot_prefix=plot_prefix,
            ...
            res = replace_key_names(
                d=epoch_test_metrics, old_term="epoch", new_term=plot_prefix
            )
            ...
            print("\r", end="")
            print_table(
                {"epoch_test_time": epoch_test_time, **epoch_test_metrics}
            )
            print()
    ...
    def clean_up(self, best_auc_epochs):
        ...
        special_epochs = list(set(best_auc_epochs + [last_epoch]))
...
class SmallWeightTrainer(Trainer):
    ...
    def _get_single_weight_parameter(self):
        single_weight_parameter = torch.tensor(
            0.5, requires_grad=self.args.soft_average, device=device
        )
        print(
            "single_weight_parameter.requires_grad:",
            single_weight_parameter.requires_grad,
        )
    ...
    def _get_optimizer(self) -> optim.Optimizer:
        ...
        return optimizer_cls(
            params=[
                {
                    "params": self.single_weight_parameter,
                    "lr": self.args.lr
                    * self.args.single_weight__lr_multiplier,
                },
                {"params": self.milnet.parameters()},
            ],
            lr=self.args.lr,
    ...
    def _run_model(
        ...
        bag_loss = self.criterion(
            bag_prediction.view(1, -1), bag_label.view(1, -1)
        )
        max_loss = self.criterion(
            max_prediction.view(1, -1), bag_label.view(1, -1)
        )
        loss = (
            self.single_weight_parameter * bag_loss
            + (1 - self.single_weight_parameter) * max_loss
        )

        with torch.no_grad():
            bag_prediction = (
                (
                    (1 - self.single_weight_parameter)
                    * torch.sigmoid(max_prediction)
                    + self.single_weight_parameter
                    * torch.sigmoid(bag_prediction)
                )
                .squeeze()
                .cpu()
                .numpy()
...
class Snuffy(SmallWeightTrainer):
    ...
    def _run_model(
        ...
        bag_prediction, loss, ins_prediction = super()._run_model(
            bag_feats, bag_label
        )

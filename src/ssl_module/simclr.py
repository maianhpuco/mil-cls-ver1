...
class ResNetSimCLR(nn.Module):
    def __init__(self, base_model, out_dim):
        super(ResNetSimCLR, self).__init__()  # Call the parent class constructor
        ...
        num_ftrs = resnet.fc.in_features  # Get the number of features from the final fully connected layer of ResNet
        ...
        self.l1 = nn.Linear(num_ftrs, num_ftrs)  # First linear layer (output size = input size)
        self.l2 = nn.Linear(num_ftrs, out_dim)  # Second linear layer (projects to output dimension)

    # Private function to get the ResNet model based on the provided name (either resnet18 or resnet50)
    def _get_basemodel(self, model_name):
        try:
            model = self.resnet_dict[model_name]  # Retrieve the model from the dictionary
            print("Feature extractor:", model_name)  # Print the name of the chosen base model
    ...
    def forward(self, x):
        ...
        h = self.features(x)
        h = h.squeeze()  # Remove any singleton dimensions (e.g., [batch_size, num_features, 1, 1] -> [batch_size, num_features])
        ...
        return h, x  # Return both the feature vector (h) and the projection vector (x)
...
class NTXentLoss(torch.nn.Module):
    ...
    def __init__(self, device, batch_size, temperature, use_cosine_similarity):
        ...
        self.softmax = torch.nn.Softmax(dim=-1)  # Softmax for turning logits into probabilities
    ...
    def _get_similarity_function(self, use_cosine_similarity):
        if use_cosine_similarity:
            self._cosine_similarity = torch.nn.CosineSimilarity(dim=-1)  # Define cosine similarity function
            return self._cosine_simililarity  # Return cosine similarity
        else:
            return self._dot_simililarity  # Return dot product similarity if cosine is not used
    ...
    def _get_correlated_mask(self):
        ...
        diag = np.eye(2 * self.batch_size)  # Identity matrix of size (2*batch_size, 2*batch_size)
        
        # Create a matrix with ones that shift diagonally by `batch_size`, marking positive pairs
        l1 = np.eye((2 * self.batch_size), 2 * self.batch_size, k=-self.batch_size)  # For left diagonal
        l2 = np.eye((2 * self.batch_size), 2 * self.batch_size, k=self.batch_size)   # For right diagonal
        ...
        return mask.to(self.device)  # Move mask to the same device as the model
    ...
    def forward(self, zis, zjs):
        ...
        l_pos = torch.diag(similarity_matrix, self.batch_size)  # Positive pairs along the left diagonal
        r_pos = torch.diag(similarity_matrix, -self.batch_size)  # Positive pairs along the right diagonal
        ...
        logits /= self.temperature  # Scale the logits by the temperature parameter

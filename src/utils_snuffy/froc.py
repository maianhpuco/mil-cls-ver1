...
def load_detections(detection_path, mask_path, level):
    ...
    detection_items = [
        (detection_row["p"], int(detection_row["y"]), int(detection_row["x"]))
        for _, detection_row in detection_table.iterrows()
    ]
    ...
    detection_items = [
        (
            detection[0],
            round(detection[1] / level_downsampling),
            round(detection[2] / level_downsampling),
        )
...
def compute_evaluation_mask(mask_path, level, include_itcs):
    ...
    image_array = mask_image.getUCharPatch(
        startX=0, startY=0, width=level_width, height=level_height, level=level
    )
    ...
    image_distance_array = scipy.ndimage.distance_transform_edt(
        input=image_negative_array
    )
    ...
    image_binary_array = np.less(
        image_distance_array, distance_threshold_pixels
    )
    ...
    image_filled_array = scipy.ndimage.binary_fill_holes(
        input=image_binary_array
    )
    # image_evaluation_mask = skimage.measure.label(input=image_filled_array, connectivity=2)
    image_evaluation_mask = skimage.measure.label(
        label_image=image_filled_array, connectivity=2
    )
    ...
    if include_itcs:
        ...
        region_properties = skimage.measure.regionprops(
            label_image=image_evaluation_mask
        )
        itc_labels = set(
            label_index + 1
            for label_index in range(len(region_properties))
            if region_properties[label_index].major_axis_length
            < itc_size_threshold
        )
...
def mp_greater_equal(args):
    ...
    if threshold == -1:
        return 0, 0
    return (
        np.greater_equal(aggregated_fps, threshold).sum(),
        np.greater_equal(aggregated_tps, threshold).sum(),
    )
...
def mp_compute_froc(froc_data, num_processes):
    ...
    #
    aggregated_fps = [
        prob for froc_item in froc_data.values() for prob in froc_item["fp"]
    ]
    aggregated_tps = [
        prob for froc_item in froc_data.values() for prob in froc_item["tp"]
    ]
    all_probs = sorted(set(aggregated_fps + aggregated_tps) - {0.0})
    image_count = len(froc_data)
    total_tumor_count = sum(
        froc_item["count"] for froc_item in froc_data.values()
    )
    ...
    with mp.Pool(num_processes) as pool:
        total_fps, total_tps = zip(
            *pool.map(
                mp_greater_equal,
                [
                    (aggregated_fps, aggregated_tps, threshold)
                    for threshold in all_probs + [-1]
                ],
            )
        )
...
def compute_score(
    average_fps, sensitivities
):  # probably should combine this with area_under_curve(x, y)
    """
    ...
    target_sum = sum(
        sensitivities_r[
            min(
                bisect.bisect_left(average_fps_r, target_fp),
                threshold_count - 1,
            )
        ]
        for target_fp in target_fp_items
    )
...
def save_results(result_file_path, average_fps, sensitivities):
    ...
    result_df = ps.DataFrame.from_dict(
        data={
            "Average FP Counts": average_fps,
            "Overall Sensitivities": sensitivities,
        },
        dtype=np.float64,
    )
    result_df.to_csv(
        path_or_buf=result_file_path,
        columns=["Average FP Counts", "Overall Sensitivities"],
        index=False,
    )
...
def collect_arguments():
    ...
    argument_parser = argparse.ArgumentParser(
        description="Compute FROC on the CAMELYON16 test set."
    )
    ...
    argument_parser.add_argument(
        ...
        required=True,
        type=str,
        help="detection file folder path",
    )
    argument_parser.add_argument(
        "-o",
        "--result",
    ...
    argument_parser.add_argument(
        "-t", "--itc", action="store_true", help="include ITCs in calculation"
    )
    argument_parser.add_argument(
        "-p", "--plot", action="store_true", help="plot curve"
    )
...
def load_detections_list(detections, mask_path, level):
    ...
    #
    detection_items = [
        (detection[0], detection[2], detection[1]) for detection in detections
    ]
...
def mp_froc_data_list(args):
    ...
    mask_path = os.path.join(
        masks_folder_path, "{image}_mask.tif".format(image=image_name)
    )
    print(f"Processing: {image_name}")
    detection_items = load_detections_list(
        detections[image_name], mask_path, evaluation_mask_level
    )
    evaluation_mask, itc_labels = (
        compute_evaluation_mask(
            mask_path=mask_path,
            level=evaluation_mask_level,
            include_itcs=include_itcs,
        )
        if slide_type == "tumor"
        else (None, set())
    )
    fp_probs, tp_probs, number_of_tumors = compute_probabilities(
...
def mp_computeFROC_list_no_cache(
    reference_file_path,
    masks_folder_path,
    detections,
    result_file_path,
    ...
    if include_itcs:
        print(
            "Warning: ITCs are included in the FROC calculation. The CAMELYON16 challenge did not include ITCs."
        )
        print("")
    ...
    with mp.Pool(num_processes) as pool:
        fp_probs, tp_probs, number_of_tumors = zip(
            *pool.map(mp_froc_data_list, mp_list)
        )
    idx = 0
    for _, reference_row in reference_table.iterrows():
        image_name, _ = os.path.splitext(reference_row["image"])
        if image_name.split(".")[0] not in images_to_calculate_for:
            continue
        froc_data[reference_row["image"]] = {
            "fp": fp_probs[idx],
            "tp": tp_probs[idx],
            "count": number_of_tumors[idx],
        }
        idx += 1
    # Compute FROC.
    #
    average_fps, sensitivities, thresholds = mp_compute_froc(
        froc_data=froc_data, num_processes=num_processes
    )

    # Compute score.
    #
    challenge_score = compute_score(
        average_fps=average_fps, sensitivities=sensitivities
    )
